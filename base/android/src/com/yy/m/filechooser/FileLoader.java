...
import com.yy.m.filechooser.utils.FileUtils;
...
public class FileLoader extends AsyncTaskLoader<List<File>> {
	...
	public FileLoader(Context context, String path, String filter) {
		super(context);
		this.mPath = path;
		this.mFilter=filter;
	}
	...
	public List<File> loadInBackground() {
		return FileUtils.getFileList(mPath, mFilter==null ? null : new FileFilter(){
			@Override
			public boolean accept(File f) {
				final String fileName = f.getName().toLowerCase();
				return f.isFile() && !fileName.startsWith(FileUtils.HIDDEN_PREFIX) && fileName.endsWith("."+mFilter.toLowerCase());
			}
